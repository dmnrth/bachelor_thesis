---
title: "04 ARMA-GARCH Forecasts high, middle and low capitalization groups"
---
...
path_folder <- "arima-garch_forecasts_H"
path <- paste0(path_folder, "/")

#description in the log file
log_descr <- "ARIMA-GARCH Forecasts: SA - HML - H, seed: 2611"
...
for(i in 0:191){
  ...
  for(j in 1:10){
    ...
    if(high[paste0(months_forward(start_date, i+t)), j] != "GM"){
      print(paste("Period no (i + 1): ", i + 1, "/192 - Stock no (j): ", j, "/10 - Progress: ", round((j + i * 10) / ( 192 * 10 ) * 100, digits = 2), "%" , sep = ""))
      
      subset <- price_data[paste0(months_forward(start_date, i), "/", months_forward(start_date, i+t)-1), high[months_forward(start_date, i+t), j]]
      ...
      x <- NULL
      x <- get_garch_forecast(subset_logreturns, 21, p, q, garch_error_distr, 2611)
      ...
      log_line <- paste0("[", Sys.time(), "] period: (", i + 1, "/192): ", months_forward(start_date, i), "/", months_forward(start_date, i+t)-1,
                         " - stock: (", j, "/10): ", high[months_forward(start_date, i+t), j], 
                         x[[2]])
...
#path for the results folder
path_folder <- "arima-garch_forecasts_M"
path <- paste0(path_folder, "/")

#description in the log file
log_descr <- "ARIMA-GARCH Forecasts: SA - HML - M, seed: 2611"
...
for(i in 0:192){
  ...
  for(j in 1:10){
    ...
    if(mid[months_forward(start_date, i+t), j] != "GM"){
      ...
      subset <- price_data[paste0(months_forward(start_date, i), "/", months_forward(start_date, i+t)-1), mid[months_forward(start_date, i+t), j]]
      ...
      log_line <- paste0("[", Sys.time(), "] period: (", i + 1, "/192): ", months_forward(start_date, i), "/", months_forward(start_date, i+t)-1,
                         " - stock: (", j, "/10): ", mid[months_forward(start_date, i+t), j], 
                         x[[2]])
...
#path for the results folder
path_folder <- "arima-garch_forecasts_L"
path <- paste0(path_folder, "/")

#description in the log file
log_descr <- "ARIMA-GARCH Forecasts: SA - HML - L, seed: 2611"
...
for(i in 0:191){ #148
  ...
  for(j in 1:10){
    ...
    if(low[months_forward(start_date, i+t), j] != "GM"){
      ...
      subset <- price_data[paste0(months_forward(start_date, i), "/", months_forward(start_date, i+t)-1), low[months_forward(start_date, i+t), j]]
      ...
      if(j == 1){
        forecasts_df <- x[[1]]
      } else if(low[months_forward(start_date, i+t), j] == "DOW"){
        tmp <- as.numeric(x[[1]])
      ...
      log_line <- paste0("[", Sys.time(), "] period: (", i + 1, "/192): ", months_forward(start_date, i), "/", months_forward(start_date, i+t)-1,
                         " - stock: (", j, "/10): ", low[months_forward(start_date, i+t), j], 
                         x[[2]])
